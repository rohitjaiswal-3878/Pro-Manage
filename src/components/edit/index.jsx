...
import { getTaskById, changeTask } from "../../apis/task";
import { useContext } from "react";
import boardContext from "../../context/dashboard";

function Edit({ editState, setEditState, taskId }) {
  ...
  useEffect(() => {
    getTaskById(taskId).then((res) => {
      if (res.status == 200) {
        ...
        if (res.data.due) {
          onChange(new Date(res.data.due));
        }
        setAssignSearch(res.data.assignTo.assignUser);
      }
    });
  }, []);
  ...
  const handleAssign = (e) => {
    ...
    setFormData({
      ...formData,
      assignTo: { ...formData.assignTo, assignUser: "" },
    });
    ...
    if (err == 0) {
      ...
      changeTask(formData).then((res) => {
        ...
        if (res.status == 200) {
          ...
          toast.error("Something went wrong while task updation!");
        }
      });
    } else {
      setErrors(msg);
    ...
    <Modal>
      {formData ? (
        ...
        >
          ...
          {/* Assign to */}
          {localStorage.getItem("userId") == formData.userId && (
